...
9. **M9 — Broker list screen.** New read-only screen, same shape as topics/groups. `kafka/admin.rs` gained `BrokerSummary { id, host, port }` and `ClusterHealth { under_replicated, offline }`, both filled from one `fetch_metadata()` call via `list_brokers()` — under-replicated = `isr().len() < replicas().len()`, offline = `leader() == -1 || error().is_some()`, computed across every partition on the cluster (no filtering of internal topics — this is cluster health, not the topic list). No `describe_cluster`/KIP-700 binding exists in rdkafka 0.39's Rust API, so there's no safe-Rust way to identify the controller broker — a "controller" badge was skipped rather than chasing a raw-protocol dead end. `Screen::BrokerList` + flat `brokers`/`broker_list_selected_index`/`cluster_health` fields on `App` (no dedicated `app/broker_list.rs` submodule — same as `topic_list`, no drill-down in v1, Enter is a no-op). `ui/screens/broker_list.rs` renders a health line ("N broker(s) · X under-replicated · Y offline", `ERROR_STYLE` when nonzero) above an ID/Host/Port table. Reachable via `b` from the topic list.
10. **M10 — Global top-level view switcher.** `1`/`2`/`3` jump directly between Topics/Groups/Brokers from any list-level screen (`TopicList`, `TopicDetail`, `GroupList`, `GroupDetail`, `BrokerList`), instead of `Esc`-ing back to topic list first to move laterally. New discrete actions `Action::SwitchToTopics`/`SwitchToGroups`/`SwitchToBrokers`, gated in `main.rs`'s `key_to_action` after the existing filter-input / replay-wizard / offset-reset-wizard hijack blocks so digits never leak into those; Producer/ExportImport/ProfileCreate are excluded so a stray digit can't blow away an in-progress draft. The reducer's `switch_top_level()` helper always re-issues the target's load command (topics/groups/brokers) even if already visited — matches the app's existing "eventual consistency, no manual cache invalidation" style — and still emits `Command::StopTail` when leaving a tailing `TopicDetail`, same as `back()`.
